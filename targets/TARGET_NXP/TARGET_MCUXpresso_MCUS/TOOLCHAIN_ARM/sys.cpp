...
extern __value_in_regs struct __initial_stackheap _mbed_user_setup_stackheap(uint32_t R0, uint32_t R1, uint32_t R2, uint32_t R3) {
    ...
    r.heap_base = (uint32_t)Image$$RW_IRAM1$$Base;
    r.heap_limit = (uint32_t)Image$$RW_IRAM1$$ZI$$Limit;
    return r;
}
...
{
    return $Super$$__rt_lib_init((unsigned) Image$$RW_IRAM1$$Base, (unsigned) Image$$RW_IRAM1$$ZI$$Limit);
}
