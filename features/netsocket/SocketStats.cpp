...
void SocketStats::stats_new_socket_entry(const Socket *const reference_id)
{
    ...
    if (get_entry_position(reference_id) >= 0) {
        ...
        //printf("Duplicate socket Reference ID %p \n", reference_id);    // TODO: delete after testing
        ...
        for (int j = 0; j < MBED_CONF_NSAPI_SOCKET_STATS_MAX_COUNT; j++) {
...
void SocketStats::stats_update_socket_state(const Socket *const reference_id, socket_state state)
{
    ...
    if (position >= 0) {
        ...
        MBED_ERROR(MBED_MAKE_ERROR(MBED_MODULE_NETWORK_STATS, MBED_ERROR_CODE_INVALID_INDEX), "State:Socket Reference ID not in list");
    }
    _mutex->unlock();
#endif
}
...
void SocketStats::stats_update_peer(const Socket *const reference_id, const SocketAddress &peer)
{
    ...
    if (position >= 0) {
        ...
        MBED_ERROR(MBED_MAKE_ERROR(MBED_MODULE_NETWORK_STATS, MBED_ERROR_CODE_INVALID_INDEX), "Peer:Socket Reference ID not in list");
    }
    _mutex->unlock();
#endif
}
...
void SocketStats::stats_update_proto(const Socket *const reference_id, nsapi_protocol_t proto)
{
    ...
    if (position >= 0) {
        ...
        MBED_ERROR(MBED_MAKE_ERROR(MBED_MODULE_NETWORK_STATS, MBED_ERROR_CODE_INVALID_INDEX), "Proto:Socket Reference ID not in list");
    }
    _mutex->unlock();
#endif
}
...
void SocketStats::stats_update_sent_bytes(const Socket *const reference_id, size_t sent_bytes)
{
    ...
    if (position >= 0) {
        ...
        MBED_ERROR(MBED_MAKE_ERROR(MBED_MODULE_NETWORK_STATS, MBED_ERROR_CODE_INVALID_INDEX), "Sent:Socket Reference ID not in list");
    }
    _mutex->unlock();
#endif
}
...
void SocketStats::stats_update_recv_bytes(const Socket *const reference_id, size_t recv_bytes)
{
    ...
    if (position >= 0) {
        ...
        MBED_ERROR(MBED_MAKE_ERROR(MBED_MODULE_NETWORK_STATS, MBED_ERROR_CODE_INVALID_INDEX), "Recv:Socket Reference ID not in list");
    }
    _mutex->unlock();
#endif
}
